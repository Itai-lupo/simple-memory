...
THROWS err_t doRseq(size_t maxRetrys, rseqCallback rseqFunc, rseqAbortHandlerCallback abortHandler, void *data)
{
  ...
  static const rseq_cs cs =  {0, 0, (uint64_t)&__start_rseq, (uint64_t)&__stop_rseq -  (uint64_t)&__start_rseq, (uint64_t)&&restart};
  ...
  QUITE_CHECK(!((uint64_t)abortHandler >=  (uint64_t)&__start_rseq && (uint64_t)abortHandler <= (uint64_t)&__stop_rseq));
...
THROWS err_t rseqMain()
{
  ...
  QUITE_RETHROW(doRseq(1000, &seq, NULL, NULL));

  LOG_INFO("rseq finished");
